...
export const upgradeSubscription = async (email, newPriceId, planType, period) => {
  try {
    ...
    // Update MongoDB immediately so the UI reflects changes instantly
    ...
    // Invalidate quota cache for the user
    if (updatedUser) {
    ...
    switch (event.type) {
      ...
      case 'customer.subscription.created': {
        ...
        if (customer.email) {
          ...
          // Get price ID from subscription to identify plan details
          const priceId =
            subscription.items?.data?.[NUMERIC_CONSTANTS.DEFAULT_VALUE]?.price?.id ||
